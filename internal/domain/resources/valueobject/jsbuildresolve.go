...
func newFSResolver(afs afero.Fs) *fsResolver {
	return &fsResolver{fs: afs, resolved: maps.NewCache[string, fs.FileMeta]()}
}

type fsResolver struct {
	fs       afero.Fs
	resolved *maps.Cache[string, fs.FileMeta]
}

func (r *fsResolver) resolveComponent(impPath string) fs.FileMeta {
	v := r.resolved.GetOrCreate(impPath, func() fs.FileMeta {
		resolve := func(name string) (fs.FileMeta, bool, bool) {
			if fi, err := r.fs.Stat(name); err == nil {
				return fi.(fs.FileMeta), true, fi.IsDir()
			}
...
func (c *BuildClient) createBuildPlugins(assetsResolver *fsResolver, depsManager identity.Manager, opts Options) ([]api.Plugin, error) {
	resolveImport := func(args api.OnResolveArgs) (api.OnResolveResult, error) {
		...
		if !isStdin {
			if strings.HasPrefix(importer, PrefixHugoVirtual) {
				...
				rel, found := c.FsService.AssetsFsMakePathRelative(importer, true)
		...
		if m != nil {
			relName, err := m.RelativeFilename()
			if err == nil {
				pi := paths.Parse(files.ComponentFolderAssets, relName)
				depsManager.AddIdentity(pi)
			}
		...
		Setup: func(build api.PluginBuild) {
			...
			build.OnLoad(api.OnLoadOptions{Filter: `.*`, Namespace: NsHugoImport},
				func(args api.OnLoadArgs) (api.OnLoadResult, error) {
					...
					if err != nil {
						fmt.Println("hugo import resolver failed to read", args.Path)
						return api.OnLoadResult{}, fmt.Errorf("js build client failed to read %q: %w", args.Path, err)
					}
					c := string(b)
