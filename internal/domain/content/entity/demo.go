...
func (s *Demo) MarshalEditor() ([]byte, error) {
	view, err := editor.Form(s,
		// Take note that the first argument to these Input-like functions
		// is the string version of each Demo field, and must follow
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to render Demo editor view: %s", err.Error())
	}

	return view, nil
}

// String defines the display name of a Demo in the CMS list-view
func (s *Demo) String() string { return s.Title }

// Create implements api.Createable, and allows external POST requests from clients
// to add content as long as the request contains the json tag names of the Demo
// struct fields, and is multipart encoded
func (s *Demo) Create(res http.ResponseWriter, req *http.Request) error {
...
// BeforeAPICreate is only called if the Demo type implements api.Createable
...
func (s *Demo) BeforeAPICreate(res http.ResponseWriter, req *http.Request) error {
...
// request. Ex. Song__pending:3 or Demo:8 depending if Demo implements api.Trustable
func (s *Demo) AfterAPICreate(res http.ResponseWriter, req *http.Request) error {
	addr := req.RemoteAddr
	log.Println("Demo sent by:", addr, "titled:", req.PostFormValue("title"))
...
func (s *Demo) Approve(res http.ResponseWriter, req *http.Request) error {
	return nil
}
...
/*
   ...
   Demo content would be in Pending, since all externally submitting Demo data
...
func (s *Demo) AutoApprove(res http.ResponseWriter, req *http.Request) error {
	...
	// is auto-approved. You could add a field to Demo, i.e.
