...
func (s *SiteLanguage) MarshalEditor() ([]byte, error) {
	view, err := editor.Form(s,
		...
		editor.Field{
			View: editor.RefSelect("Language", s, map[string]string{
				"label": "Language",
			},
				"Language",
				`{{ .name }} `,
		...
		editor.Field{
			View: editor.Checkbox("Default", s, map[string]string{
		...
		editor.Field{
			View: editor.Input("Folder", s, map[string]string{
				"label":       "Folder",
				"type":        "text",
				"placeholder": "Enter the language folder name here",
			}),
		},
	)
...
func (s *SiteLanguage) String() string {
	...
	l, _ := extractTypeAndID(s.Language)
