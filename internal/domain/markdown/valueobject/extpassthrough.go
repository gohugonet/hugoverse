...
func newPassthroughInline(segment text.Segment, delimiters *PassThroughDelimiters) *passthroughInline {
...
type inlinePassthroughParser struct {
	PassthroughDelimiters []PassThroughDelimiters
}

func newInlinePassthroughParser(ds []PassThroughDelimiters) parser.InlineParser {
...
func getFullOpeningDelimiter(delims []PassThroughDelimiters, line []byte) *PassThroughDelimiters {
...
func openersFirstByte(delims []PassThroughDelimiters) []byte {
	var firstBytes []byte
...
func containsDelimiters(delims []PassThroughDelimiters, toFind *PassThroughDelimiters) bool {
	for _, d := range delims {
...
func newPassthroughInlineTransformer(ds []PassThroughDelimiters) parser.ASTTransformer {
...
func NewPassThroughExt(c PassThroughConfig) goldmark.Extender {
	...
	combinedDelimiters := make([]PassThroughDelimiters, len(c.InlineDelimiters)+len(c.BlockDelimiters))
