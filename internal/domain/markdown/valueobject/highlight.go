...
func (h chromaHighlighter) HighlightCodeBlock(ctx markdown.CodeblockContext, opts any) (markdown.HighlightResult, error) {
	cfg := h.cfg

	var b strings.Builder

	attributes := ctx.(markdown.AttributesOptionsSliceProvider).AttributesSlice()
...
func (h chromaHighlighter) RenderCodeblock(cctx context.Context, w pio.FlexiWriter, ctx markdown.CodeblockContext) error {
	cfg := h.cfg
...
func highlight(fw pio.FlexiWriter, code, lang string, attributes []markdown.Attribute, cfg HighlightConfig) (int, int, error) {
...
func writeDivStart(w pio.FlexiWriter, attrs []markdown.Attribute) {
...
func writeDivEnd(w pio.FlexiWriter) {
	w.WriteString("</div>")
}

type byteCountFlexiWriter struct {
	delegate pio.FlexiWriter
	counter  int
}
