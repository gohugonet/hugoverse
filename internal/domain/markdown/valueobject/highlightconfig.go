...
func (cfg HighlightConfig) toHTMLOptions() []html.Option {
...
func applyOptions(opts any, cfg *HighlightConfig) error {
	if opts == nil {
		return nil
	}
	switch vv := opts.(type) {
...
func applyOptionsFromString(opts string, cfg *HighlightConfig) error {
...
func applyOptionsFromMap(optsm map[string]any, cfg *HighlightConfig) error {
	normalizeHighlightOptions(optsm)
	return mapstructure.WeakDecode(optsm, cfg)
}

func applyOptionsFromCodeBlockContext(ctx markdown.CodeblockContext, cfg *HighlightConfig) error {
	if cfg.LineAnchors == "" {
