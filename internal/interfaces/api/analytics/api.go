...
// RANGE determines the number of days hugoverse request analytics and metrics are
...
// after call to Setup() from the same place.
func Close() {
	err := store.Close()
	if err != nil {
		log.Println(err)
...
// Setup creates a db connection, initializes the db with schema and data and
// sets up the queue/batching channel
func Setup(dataDir string) {
	var err error
	analyticsDb := filepath.Join(dataDir, "analytics.db")
